...
const steps = ['Plan your course overview', 'Create course content', 'Describe your course', 'Publish Your Course'];
...
export default function CreateCourse(props) {
	...
	const trySubmit = async () => {
		...
		await fetch('/course/create', requestOptions)
		...
		setSubmitting(false);
		dispatch(clearAll());
		localStorage.setItem('courseCreated', 'Course has been created successfully!');
	...
	return (
		<Container noSearch hasStepper heading='Create a new Course' steps={steps} handleStep={handleStep} activeStep={activeStep}>
				...
				<motion.div style={stepFooterStyle}>
					...
					<Button onClick={handleNext} disabled={!status.success} variant="outline-secondary" style={{width: 90, height: 40, fontSize: 16}}>
						<motion.div style={{display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
							...
							<motion.span>{activeStep === steps.length - 1 ? 'Finish' : 'Next'}</motion.span>
